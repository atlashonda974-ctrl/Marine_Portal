...
if (!$conn) {
 ...
 $client_code = $_GET['client_code'];
	...
	$codes = explode(',', $client_code);
	$client_arr = "'" . implode("', '", $codes) . "'";
						...
						WHERE uw_docheader.GDH_EXPIRYDATE >= SYSDATE AND uw_docheader.GDH_ISSUEDATE <= SYSDATE
						AND uw_docheader.PDT_DOCTYPE = 'O'
						AND uw_docheader.PPS_PARTY_CODE IN ($client_code)
						ORDER BY uw_docheader.GDH_ISSUEDATE
					";
