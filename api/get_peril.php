...
if (!$conn) {
 ...
 $year = $_GET['year'];
						...
						$query ="SELECT GAC_ACTION_CODE, PTF_SHORTDESC, GAC_RATE 
						FROM ailmis.uw_actiondtl 
						Where PLC_LOC_CODE = '$loc' 
						AND PDT_DOCTYPE = '$type'
						AND GDH_DOCUMENTNO = '$doc'
						AND GDH_YEAR = '$year'
						AND PDP_DEPT_CODE = '12'";
						...
						$combo = oci_parse($conn, $query);
												...
												$posts_arr = array();
                                                  while($res = oci_fetch_array($combo, OCI_ASSOC+OCI_RETURN_NULLS)) {
														 ...
														 $post_item = array(
																'STATUS' => "Exist",	
																'GAC_ACTION_CODE' => ($res['GAC_ACTION_CODE'] !== null ? $res['GAC_ACTION_CODE'] : "&nbsp;"),
																'PTF_SHORTDESC' => ( $res['PTF_SHORTDESC'] !== null ? $res['PTF_SHORTDESC'] : "&nbsp;"),
																'GAC_RATE' => ($res['GAC_RATE'] !== null ? $res['GAC_RATE'] : "&nbsp;")
