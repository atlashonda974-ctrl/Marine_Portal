 ...
 document.addEventListener('DOMContentLoaded', function() {
        ...
        if (packingSelect && otherPackingInput && otherPackingText) {
            packingSelect.addEventListener('change', function() {
                console.log('Select value changed:', this.value); // Let's see if this logs
                if (this.value === 'Other') {
            ...
            console.log('Event listener attached to packingSelect'); // Confirm listener attachment
        } else {
            console.error('One or more elements not found!'); // Check if elements are correctly referenced
        }
});
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (foreignCurrencyInput && incidentalPercentageInput && incidentalChargesInput && tolerancePercentageInput && toleranceChargesInput) {
        // Event listener for Foreign Currency input (for Incidental and Tolerance Charges)
        foreignCurrencyInput.addEventListener('input', function() {
            ...
            if (!isNaN(foreignCurrencyValue)) {
                // Incidental Charges Calculation
                ...
                // Tolerance Charges Calculation (based on Foreign Currency)
                tolerancePercentageInput.value = 10; // Example: Set Tolerance to 5% initially
        ...
        // Event listener for Incidental Percentage input
        incidentalPercentageInput.addEventListener('input', function() {
            ...
            if (!isNaN(incidentalPercentageValue) && !isNaN(foreignCurrencyValue)) {
                ...
                const fcic = incidentalCharge + foreignCurrencyValue;
                tolerancePercentageInput.value = 10; // Set to 10%
                const toleranceCharge = fcic * 0.10;
                toleranceChargesInput.value = toleranceCharge.toFixed(0); 
        ...
        // Event listener for Tolerance Percentage input (Corrected Logic)
        tolerancePercentageInput.addEventListener('input', function() {
            const tolerancePercentageValue = parseFloat(this.value);
            const foreignCurrencyValue = parseFloat(foreignCurrencyInput.value); // Use Foreign Currency for calculation
...
//Set Current date to Application Date Field
document.addEventListener('DOMContentLoaded', function() {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    currencyTypeSelect.addEventListener('change', function() {
        ...
        if (selectedCurrency) {
            ...
            exchangeRateInput.value = ''; // Clear exchange rate if no currency is selected
        }
    });
});
...
function updateVesselCode() {
    ...
    // If no matching description is found, clear the code field
    codeInput.value = '';
}
...
function filterFunction(input, event) {
    ...
    // Prevent cursor from jumping to the end on arrow keys
...
// Disable initially unchecked peril_per fields (in case you don't want them all enabled by default)
document.addEventListener('DOMContentLoaded', function() {
...
function calculatePremium(perilPerInput, perilChrgId, siRsId) {
    ...
    if (!isNaN(perilPercentage) && sumInsuredInput && calculatedPremiumInput) {
        ...
        if (!isNaN(sumInsured)) {
            ...
            calculatedPremiumInput.value = calculatedPremium.toFixed(0); // Round to 2 decimal places
        } else {
            calculatedPremiumInput.value = ''; // Clear if Sum Insured is not a number
        }
    } else {
        calculatedPremiumInput.value = ''; // Clear if Percentage is not a number or elements not found
    }
}
...
function calculateGSTAndStampDutyAndNet() {
    ...
    if (regionSelect.value === 'Sindh') {
        gstRate = 0.15; // 15%
        stampDutyRate = 0.00006; // 0.006%
    } else if (regionSelect.value === 'Punjab') {
        gstRate = 0.16; // 16%
        stampDutyRate = 0.00006; // 0.003%
    }
    ...
    const fifAmount = subTotal * 0.01; // 1% of Sub Total
...
<label>MOP</label>
<input type="text" class="form-control" placeholder="MOP" name="mop" id="mop" value="{{ $openPolData[0]->GDH_DOC_REFERENCE_NO }}" style="background-color: #f0f0f0;" readonly>
</div>

<div class="form-group col-md-4">
<label>Portal Number</label>
<input type="text" class="form-control" placeholder="Portal Number" name="portal_no" id="portal_no" value="{{ $pol[0]->portal_no }}" style="background-color: #f0f0f0;" readonly>
</div>
...
<input type="text" class="form-control" placeholder="Business Class" name="bus_class" id="bus_class" value="{{ $openPolData[0]->PBC_DESC }}" style="background-color: #f0f0f0;" readonly>
</div>


<div class="form-group col-md-4">
<label>Insured Name</label>
<input type="text" class="form-control" placeholder="Insured Name" name="insured" id="insured" value="{{ $openPolData[0]->PPS_DESC }}" style="background-color: #f0f0f0;" readonly>
</div>
</div>
...
<label>Address</label>
<textarea class="form-control" placeholder="Address" name="address" style="background-color: #f0f0f0;" readonly>{{ $openPolData[0]->PAS_ADDRESS1 }}</textarea>
</div>
</div>
...
<input type="number" class="form-control"  placeholder="Per Carry Limit"  name="per_carry" id="per_carry" value="{{ $pol[0]->per_carry }}" readonly>
</div>
</div>
...
<div class="form-group col-md-4">
<label>PO No.</label>
<input type="number" class="form-control" placeholder="PO No" name="po_no" id="po_no" value="{{ $pol[0]->po_no }}" readonly>
</div>

<div class="form-group col-md-4">
<label>Voyage/Consignment/Flight No.</label>
<input type="number" class="form-control" placeholder="Voyage/Consignment/Flight No." name="voyage_no" id="voyage_no" value="{{ $pol[0]->voyage_no }}" readonly>
</div>

<div class="form-group col-md-4">
<label>Arrival Date</label>
<input type="date" class="form-control" placeholder="Arrival Date" name="arr_date" id="arr_date" value="{{ $pol[0]->arr_date }}" readonly>
</div>
...
<input type="number" class="form-control" placeholder="Invoice No" name="inv_no" id="inv_no" value="{{ $pol[0]->inv_no }}" readonly>
</div>


<div class="form-group col-md-4">
<label>Invoice Date</label>
<input type="date" class="form-control" placeholder="Invoice Date" name="inv_date" id="inv_date" value="{{ $pol[0]->inv_date }}" readonly>
</div>
...
<input type="number" class="form-control" placeholder="BL/AWB No" name="bl_no" id="bl_no" value="{{ $pol[0]->bl_no }}" readonly>
</div>


<div class="form-group col-md-4">
<label>BL/AWB Date</label>
<input type="date" class="form-control" placeholder="BL/AWB Date" name="bl_date" id="bl_date" value="{{ $pol[0]->bl_date }}" readonly>
</div>
...
<input type="text" class="form-control" placeholder="Voyage From" name="voyage_from" id="voyage_from" value="{{ $pol[0]->voyage_from }}" readonly>
</div>

<div class="form-group col-md-4">
<label>Voyage To</label>
<input type="text" class="form-control" placeholder="Voyage To" name="voyage_to" id="voyage_to" value="{{ $pol[0]->voyage_to }}" readonly>
</div>

<div class="form-group col-md-4">
<label>VIA</label>
<input type="text" class="form-control" placeholder="VIA" name="via" id="via" value="{{ $pol[0]->via }}" readonly>
</div>
...
<div class="searchable">
    <input type="text" autocomplete="off" list="vessels" id="vessel"  name="vessel" class="form-control" placeholder="search vessel" onchange="updateVesselCode()"  onkeyup="filterFunction(this,event)" onchange="doValidateCity();" value="{{ $pol[0]->vessel }}" readonly>
    <input type="hidden" id="vessel_code" name="vessel_code">
    <datalist id="vessels">
	@foreach($vessels as $row)
		<option value="{{ $row->PVC_CODE }}">{{ $row->PVC_DESC }}</option>
    @endforeach
    </datalist>
</div>
</div>
...
<input type="number" class="form-control" placeholder="LC Number" name="lc_no" id="lc_no" value="{{ $pol[0]->lc_no }}" readonly>
</div>

<div class="form-group col-md-4">
<label>LC Date</label>
<input type="date" class="form-control" placeholder="LC Date" name="lc_date" id="lc_date" value="{{ $pol[0]->lc_date }}" readonly>
</div>
</div>
...
<input type="date" class="form-control" placeholder="Salling/Depature/Dispatch on/or about" name="salling" id="salling" value="{{ $pol[0]->salling }}" readonly>
</div>
</div>
...
<input type="number" class="form-control" placeholder="Form E/Builty Number" name="builty_no" id="builty_no" value="{{ $pol[0]->builty_no }}" readonly>
</div>

<div class="form-group col-md-4">
<label>Builty Date</label>
<input type="date" class="form-control" placeholder="Builty Date" name="builty_date" id="builty_date" value="{{ $pol[0]->builty_date }}" readonly>
</div>
</div>
...
<textarea class="form-control" placeholder="Additional Terms (if any)" name="add_terms" value="{{ $pol[0]->add_terms }}" readonly> </textarea>
</div>

<div class="form-group col-md-6">
<label>Subject Matter Insured</label>
<textarea class="form-control" placeholder="Subject Matter Insured" name="sub_mat" value="{{ $pol[0]->sub_mat }}" readonly> </textarea>
</div>
</div>
...
<input type="number" class="form-control" placeholder="Exchange Rate" name="ex_rate" id="ex_rate" value="{{ $pol[0]->ex_rate }}" style="background-color: #f0f0f0;" readonly>
</div>
...
<input type="number" class="form-control" placeholder="Foreign Currency" name="si_fc" id="si_fc" value="{{ $pol[0]->si_fc }}" readonly>
</div>
</div>
...
<div class="form-group col-md-6">
    ...
    <div class="row">
        <div class="col-md-2">
            <input type="number" class="form-control" placeholder="%" name="inc_per" id="inc_per" value="{{ $pol[0]->inc_per }}" readonly>
        </div>
        <div class="col-md-6">
            <input type="number" class="form-control" placeholder="Incidental Charges" name="inc_chrg" id="inc_chrg" value="{{ $pol[0]->inc_chrg }}" style="background-color: #f0f0f0;" readonly>
        </div>
    </div>
</div>
...
<div class="form-group col-md-6">
    ...
    <div class="row">
        <div class="col-md-2">
            <input type="number" class="form-control" placeholder="%" name="tol_per" id="tol_per" value="{{ $pol[0]->tol_per }}" readonly>
        </div>
        <div class="col-md-6">
            <input type="number" class="form-control" placeholder="Tolerance Charges" name="tolrence" id="tolrence" value="{{ $pol[0]->tolrence }}"  style="background-color: #f0f0f0;" readonly>
        </div>
    </div>
</div>
</div>
...
<input type="number" class="form-control" placeholder="Total Foreign Currency Value" name="si_tfc" id="si_tfc" value="{{ $pol[0]->si_tfc }}" style="background-color: #f0f0f0;" readonly>
</div>
...
<input type="number" class="form-control" placeholder="Sum Insured (Rs)" name="si_rs" id="si_rs" value="{{ $pol[0]->si_rs }}" style="background-color: #f0f0f0;" readonly>
</div>
</div>
...
<input type="number" class="form-control" placeholder="Gross Premium" name="gross_pre" id="gross_pre" style="background-color: #f0f0f0;" readonly>
</div>
</div>
...
<div class="form-group col-md-4">
<label>Admin</label>
<input type="number" class="form-control" placeholder="Admin" name="admin_sur" id="admin_sur" style="background-color: #f0f0f0;" readonly>
</div>

<div class="form-group col-md-4">
<label>Sub Total</label>
<input type="number" class="form-control" placeholder="Sub Total" name="sub_total" id="sub_total" style="background-color: #f0f0f0;" readonly>
</div>
</div>
...
<div class="form-group col-md-4">
<label>GST</label>
<input type="number" class="form-control" placeholder="GST" name="gst" id="gst" style="background-color: #f0f0f0;" readonly>
</div>
</div>
...
<div class="form-group col-md-4">
<label>F.I.F</label>
<input type="number" class="form-control" placeholder="F.I.F" name="fif" id="fif" style="background-color: #f0f0f0;" readonly>
</div>

<div class="form-group col-md-4">
<label>Stamp Duty</label>
<input type="number" class="form-control" placeholder="Stamp Duty" name="stamp_duty" id="stamp_duty" style="background-color: #f0f0f0;" readonly>
</div>

<div class="form-group col-md-4">
<label>Net Premium</label>
<input type="number" class="form-control" placeholder="Net Premium" name="net_pre" id="net_pre" style="background-color: #f0f0f0;" readonly>
</div>
</div>
